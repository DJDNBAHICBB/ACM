...
inline bool get_which(int x){       //判断是否是右孩子
    return sons[f[x]][1]==x;
}
...
inline void rotate(int x){
    ...
    sons[father][which_son]=sons[x][which_son^1];  //f的右孩子取x的左孩子（此时x为右），反之亦然
    f[sons[father][which_son]]=father;        //将刚刚换的节点连上
    sons[x][which_son^1]=father;    //x成为原f的f
    f[father]=x;        //f的f变为x
    f[x]=g_father;      //x的f变为gf
    if(g_father){
        sons[g_father][sons[g_father][1]==father]=x;
    }
    update(father);         //只有f和x的size有变化
    update(x);
}
inline void splay(int x){
    for (int fa;fa=f[x];rotate(x))
        if (f[fa])
            rotate((get_which(x)==get_which(fa))?fa:x);    //判断是否是一条线
    root=x;
}
inline void insert(int x){
    ...
    while(1){
        ...
        fa=now;
        now=sons[now][value[now]<x];         //寻找应在何处
        if(!now){
            whole_size++;
...
inline int find_pre(){          //root = x
    int now=sons[root][0];
    while(sons[now][1])now=sons[now][1];
    return now;
}
inline int find_suffix(){       //root = x
    int now=sons[root][1];
